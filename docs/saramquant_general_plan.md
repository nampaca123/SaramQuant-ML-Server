...
| 미국 | NASDAQ Screener API (`NasdaqScreenerClient`) | NYSE/NASDAQ 벌크 조회, 병렬 fetch (~6초) |
...
- `User-Agent` 헤더 필수, `download=true` 파라미터 필수
- 인증 불필요
...
│      │    (non-blocking, sector IS NULL 대상만)             │
...
| 2 | KR 섹터 수집 | pykrx 업종 인덱스 매핑 (non-blocking) |
| 3 | KR 일봉 수집 | pykrx로 당일 전 종목 OHLCV |
| 4 | KR 벤치마크 수집 | pykrx로 KOSPI/KOSDAQ 지수 |
| 5 | KR 무위험금리 수집 | ECOS API (91D, 3Y, 10Y) |
| 6 | KR 지표 계산 | 전 종목 23개 지표 벌크 계산 → stock_indicators |
| 7 | KR 펀더멘털 계산 | 재무제표 기반 7개 지표 → stock_fundamentals |
...
| 2 | US 섹터 수집 | NASDAQ Screener API 벌크 조회, ~6초 (non-blocking) |
| 3 | US 일봉 수집 | Alpaca API 배치 수집 |
| 4 | US 벤치마크 수집 | yfinance로 S&P500/NASDAQ 지수 |
| 5 | US 무위험금리 수집 | FRED API (91D, 1Y, 3Y, 10Y) |
| 6 | US 지표 계산 | 전 종목 23개 지표 벌크 계산 → stock_indicators |
| 7 | US 펀더멘털 계산 | 재무제표 기반 7개 지표 → stock_fundamentals |
